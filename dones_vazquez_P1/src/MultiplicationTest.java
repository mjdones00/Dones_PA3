...
public class MultiplicationTest {
    ...
    private static double userMultInput;
...
private String pseudoRanPositive(SecureRandom rand){
    ...
    public int generateNewTest(Scanner scnr, SecureRandom rand, int numRuns, int numSize) {
        ...
        double mult = (double) num1 * num2;

        System.out.println("How much is " + num1 + " times " + num2 + "?");
            userMultInput = scnr.nextInt();
            if (Double.compare(userMultInput, mult) == 0) {
                System.out.println(pseudoRanPositive(rand));
                rightCounter++;
            }
            else if (Double.compare(userMultInput, mult) !=0) {
